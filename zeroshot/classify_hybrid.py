...
OUTPUT_CSV = "comments_classified_hybrid.csv"
CHECKPOINT_EVERY = 50
ROW_START = None
ROW_END = None
...
def classify_sentiment(classifier, uid, comment):
    try:
        out = classifier(comment[:512], truncation=True)[0]
        label = out["label"].lower()
        return uid, label == "negative", label, round(out["score"], 4)
    except Exception:
...
def main():
    ...
    def checkpoint():
        scores = pd.DataFrame(results).drop_duplicates(subset="uid", keep="last")
        scored_ids = set(scores["uid"].astype(str))
        batch = remaining[remaining["uid"].astype(str).isin(scored_ids)].copy()
        mapping = scores.set_index("uid")[["negative", "sentiment_label", "sentiment_score"]].to_dict("index")
    ...
    for _, row in tqdm(remaining.iterrows(), total=len(remaining), desc="Classifying"):
        uid, negative, label, score = classify_sentiment(
            classifier, row["uid"], str(row["comment"])
        )
        results.append({
            "uid": uid,
            "negative": negative,
